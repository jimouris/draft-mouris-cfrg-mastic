...
with a chosen prefix. This functionality enables two classes of applications.
First, Mastic allows partitioning metrics data by client attributes without
revealing which clients have which attributes. Second, it supports a more
general variant of the heavy hitters problem, where the goal is to compute the
subset of inputs that occur most frequently. In particular, Mastic allows
computing the subset of inputs that have the highest total weight.
...
The private "heavy hitters" problem is to compute the most popular inputs
generated by clients without learning the inputs themselves. For example, a
browser vendor might want to know which websites are visited most frequently
without learning which clients visited which websites.
...
"functionality gives rise to two types of application:

1. "weighted heavy hitters": Rather than compute the most frequent inputs, the
   goal here is to compute the set of inputs with the highest total weight. For
   example, a web browser might want to know which websites have the highest
   average load time. This problem is a generalization of heavy hitters,
   meaning Mastic can be used as a drop-in replacement for Poplar1. Mastic is
...
1. "attribute-based metrics": The Prio3 VDAF ({{Section 7 of !VDAF}}) can be
   ...
   this functionality without revealing any client's attribute.
...
the main building block of Poplar1. IDPF is a form of function secret sharing
{{BGI15}}, where a client generates shares of a secret function `F` such that
each server can computes shares of `F(x)` for a chosen `x`. In our case, the
function being shared is the function `F` associated with a secret input string
...
weight. For example, if the weights represent page load times in seconds, the
data collector will want to make sure that each load time falls within some
expected range, say within 30 seconds. Otherwise, misbehaving clients can try
to poison the computation by reporting out of range values. This range check is
accomplished with the Fully Linear Proof (FLP) system of {{Section 7.3 of
!VDAF}}. An FLP allows properties of secret share data to be validated without
revealing the data itself. In Mastic, the client generates an FLP of its
`beta`'s validity; when the servers are ready to evaluate the VIDPF, they first
compute shares of `beta` and verify the FLP. Then the VIDPF ensures that the
same `beta` for each subsequent VIDPF evaluation.
...
!VDAF}} in particular the following functions:

* `gen_rand(len: int) -> bytes`
* TODO List all functions we use in alphabetical order.

This document uses the following terms as defined in {{Section 5 of !VDAF}}:
"Aggregator",
"Client",
"Collector",
"aggregate result",
...
An instance of Mastic is associated with an instance of an FLP as defined in
{{Section 7.1 of !VDAF}}. The Client's measurement has two components: the
input string `alpha` of type `int` (TODO: The type may change once we solve
issue \#34) and its weight `beta` whose type is determined by the FLP. The
aggregate result is a list of values, one for each candidate prefix; and the
type of each vvalue is determined by the FLP.

Mastic also makes use of NTT-friendly finite fields as specified in {{Section
6.1 of !VDAF}} and eXtendable Output Functions (XOFs) as specified in {{Section
6.2 of !VDAF}}.
...
({Section 7.1 of !VDAF}}) for this purpose. ({{MST24}} describe a simple
`range(2)` check, but we would like more sophisticated range checks for
Mastic.)
