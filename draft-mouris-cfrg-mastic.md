...
denoted `BITS`, and an instance of the FLP in {{Section 7.3 of !VDAF}}. The
Client's measurement has two components: the input string `alpha: int` in
`range(2**BITS)` (TODO: The type may change once we solve issue \#34) and its
weight `beta: Meaasurement` where `Measurement` is is determined by the FLP
({{Section 7.1 of !VDAF}}). The aggregate result has type `list[AggResult]`,
where `AggResult` is likewise a type dfined by the FLP. Each element of this
list corresponds to the total wieght for one of the candidate prefixes.
