...
from common import TEST_VECTOR, VERSION, Bytes, Unsigned, format_dst, zeros, \
                   from_le_bytes, gen_rand, next_power_of_2, \
                   print_wrapped_line, to_be_bytes, to_le_bytes, xor, concat
...
class Prg:
    ...
    def next_vec_ring(self, Ring, length: Unsigned):
        ...
        vec = []
        while len(vec) < length:
            x = from_le_bytes(self.next(Ring.ENCODED_SIZE))
            x &= m
            if x < Ring.MODULUS:
                vec.append(Ring.new_elm(x))
        return vec
